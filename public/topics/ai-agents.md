---
title: "🤖 AI Agents — Human-in-the-Loop Research"
---

<div class="hitl-topic-page">
...
<script>
// AI Agent concepts for the quick grid
const agentConcepts = [
  { name: 'AI Agent', papers: 45, desc: 'Autonomous systems with goals' },
  { name: 'Multi-Agent Systems', papers: 28, desc: 'Cooperating agent teams' },
  { name: 'Tool Use', papers: 35, desc: 'Agents invoking external tools' },
  { name: 'Planning', papers: 30, desc: 'Task decomposition & scheduling' },
  { name: 'GUI Agents', papers: 22, desc: 'Interface interaction agents' },
  { name: 'Memory Systems', papers: 18, desc: 'Context & knowledge storage' },
  { name: 'Reinforcement Learning', papers: 15, desc: 'Learning from rewards' },
  { name: 'Embodied Agents', papers: 12, desc: 'Physical world interaction' }
];

// Render concept quick grid
...
// Count user contributions for this topic
function countContributions() {
  let count = 0;
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key.startsWith('wiki_contributions_') || key.startsWith('concept_validation_') || key.startsWith('feedback_')) {
      const data = JSON.parse(localStorage.getItem(key) || '[]');
      if (Array.isArray(data)) count += data.length;
      else count += 1;
    }
  }
  const el = document.getElementById('hitlContribCount');
